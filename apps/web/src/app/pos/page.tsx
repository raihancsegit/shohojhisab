...
export default function PosPage() {
  ...
  // ⚡ Smart Voice & Express Memo Entry State
  const [expressInput, setExpressInput] = useState('');
  const [isExpressListening, setIsExpressListening] = useState(false);
  const [expressPreview, setExpressPreview] = useState<any | null>(null);
  const expressRecognitionRef = useRef<any>(null);
  ...
  // Restore saved POS mode or check URL param
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const modeParam = params.get('mode');
      if (modeParam === 'numpad' || modeParam === 'calculator') {
        setPosMode('numpad');
      } else if (modeParam === 'catalog') {
        setPosMode('catalog');
      } else {
  ...
  const changePosMode = (mode: 'catalog' | 'numpad') => {
    setPosMode(mode);
    if (typeof window !== 'undefined') {
      localStorage.setItem('sh_pos_mode', mode);
    }
  };
  ...
  // 🔢 Expression Parser (supports "4×12" or "140")
  const parseNumpadExpression = (input: string) => {
    ...
    if (clean.includes('*')) {
      ...
      if (!isNaN(q) && !isNaN(p) && q > 0 && p > 0) {
        return { quantity: q, unitPrice: p, total: Math.round(q * p * 100) / 100, isMultiplier: true };
      }
      if (!isNaN(q) && q > 0) {
        return { quantity: q, unitPrice: 0, total: 0, isMultiplier: true };
      }
    }
    const val = parseFloat(clean);
    if (!isNaN(val) && val > 0) {
      return { quantity: 1, unitPrice: val, total: val, isMultiplier: false };
    }
    return { quantity: 1, unitPrice: 0, total: 0, isMultiplier: false };
  };

  const parsedPending = parseNumpadExpression(numpadInput);
  ...
  const popularNumpadProducts = useMemo(() => {
    return products.filter(p => Number(p.stock || 0) > 0).slice(0, 6);
  }, [products]);
  ...
  const handleNumpadDigit = (digit: string) => {
    ...
    if (digit === '×') {
      if (numpadInput.includes('×') || numpadInput === '') return;
    }
  ...
  const handleNumpadMultiplier = (mult: number) => {
    ...
    if (!numpadInput) {
      ...
      setNumpadInput(`${mult}×${parts[1] || ''}`);
    } else {
      setNumpadInput(`${mult}×${numpadInput}`);
    }
  };
  ...
  const handleNumpadAddCurrent = (overrideProduct?: any) => {
    const targetProd = overrideProduct || numpadSelectedProduct;
    const expr = parseNumpadExpression(numpadInput);

    let amt = expr.total;
    let qty = expr.quantity || 1;
    let uPrice = expr.unitPrice || amt;

    if ((!amt || amt <= 0) && targetProd) {
      uPrice = Number(targetProd.sellingPrice) || 0;
      amt = Math.round(qty * uPrice * 100) / 100;
    }
  ...
  const handleNumpadAddQuickAmount = (amt: number) => {
    ...
    setNumpadItems(prev => [
      ...prev,
      {
        id: 'np-' + Date.now() + Math.random().toString(36).slice(2, 6),
        amount: amt,
        note: numpadNote.trim() || `আইটেম ${prev.length + 1}`,
        quantity: 1,
        unitPrice: amt,
        unit: 'পিস'
      }
  ...
  const handleNumpadCheckout = async (method: 'cash' | 'due') => {
    ...
    if (numpadPendingVal > 0) {
      const pMatch = numpadSelectedProduct;
      const expr = parseNumpadExpression(numpadInput);
      const qty = expr.quantity || 1;
      const uPrice = expr.unitPrice || numpadPendingVal;
      const baseNote = numpadNote.trim() || (pMatch ? (pMatch.banglaName || pMatch.name) : `আইটেম ${finalItems.length + 1}`);
      finalItems.push({
        id: 'np-pending',
        amount: numpadPendingVal,
        note: `${baseNote}${qty > 1 && !baseNote.includes(`(${qty}`) ? ` (${qty} ${pMatch?.unit || 'পিস'})` : ''}`,
        productId: pMatch?.id || null,
        unit: pMatch?.unit || 'আইটেম',
        quantity: qty,
        unitPrice: uPrice,
    ...
    const payload = {
      ...
      items: finalItems.map(i => {
        const pMatch = i.productId ? products.find(p => p.id === i.productId) : null;
        const qty = i.quantity || 1;
        const uPrice = i.unitPrice || (i.amount / qty);
        const pPrice = i.purchasePrice || (pMatch?.purchasePrice ? Number(pMatch.purchasePrice) : Math.round(uPrice * 0.85));
        return {
          productId: i.productId || null,
          productName: i.note ? i.note : 'দ্রুত নগদ/বাকি বিক্রি',
          quantity: qty,
          selectedUnit: i.unit || pMatch?.unit || 'আইটেম',
          sellingPrice: uPrice,
  ...
  const handleNumpadPushToCart = () => {
    ...
    finalItems.forEach(it => {
      ...
      const uPrice = it.unitPrice || (it.amount / qty);
      const customProd = pMatch || {
        ...
        sellingPrice: uPrice,
        purchasePrice: it.purchasePrice || Math.round(uPrice * 0.85),
        unit: it.unit || 'আইটেম',
        stock: 999
      };
    ...
    setNumpadSelectedProduct(null);
    setShowNumpadProductDropdown(false);
    changePosMode('catalog');
  };

  // ⌨️ Physical Keyboard Numpad shortcuts when posMode === 'numpad'
  useEffect(() => {
    const handleKeypadKey = (e: KeyboardEvent) => {
      if (posMode !== 'numpad') return;
      ...
      if (/^[0-9]$/.test(e.key)) {
        ...
        if (numpadPendingVal > 0) {
          handleNumpadAddCurrent();
        } else if (numpadTotal > 0) {
          handleNumpadCheckout('cash');
        }
      }
    };

    window.addEventListener('keydown', handleKeypadKey);
  ...
  const addToCart = (product: any, qty: number = 1, unit?: string, customPrice?: number) => {
    ...
    let baseRate = customPrice !== undefined ? customPrice : (Number(product.sellingPrice) || 0);
    if (customPrice === undefined) {
      if (subUnit && itemUnit === subUnit && ratio > 0) {
        ...
        } else if (primaryUnit === 'পাতা' && (itemUnit === 'ট্যাবলেট' || itemUnit === 'ক্যাপসুল' || itemUnit === 'পিস' || itemUnit === 'টা')) {
  ...
  // ⚡ Express Natural Language Item Parser & Add Handler (ভয়েস বা লিখে সরাসরি মেমোতে আইটেম যোগ)
  const handleAddExpressItem = (textToParse?: string) => {
    const rawText = (textToParse !== undefined ? textToParse : expressInput).trim();
    if (!rawText) return;

    try {
      const res = parseVoicePOSCommand(rawText, products);

      // 1. Remove Item Command (e.g. "শেষেরটা কাটো", "তেল বাদ দাও")
      if (res.type === 'remove_item') {
        if (posMode === 'calculator' || posMode === 'numpad') {
          if (numpadItems.length === 0) {
            setVoiceNotice('⚠️ মেমোতে কোনো আইটেম নেই।');
            return;
          }
          if (res.removeItemName === '__last__' || !res.removeItemName) {
            const removed = numpadItems[numpadItems.length - 1];
            setNumpadItems(prev => prev.slice(0, -1));
            playBeep(600);
            triggerHaptic('light');
            setVoiceNotice(`✓ সর্বশেষ আইটেমটি (${removed.note || `৳${removed.amount}`}) বাদ দেওয়া হয়েছে।`);
            speakAnnouncement('শেষের আইটেমটি বাদ দেওয়া হয়েছে।');
          } else {
            const q = res.removeItemName.toLowerCase();
            const target = numpadItems.find(i => (i.note || '').toLowerCase().includes(q));
            if (target) {
              setNumpadItems(prev => prev.filter(i => i.id !== target.id));
              playBeep(600);
              triggerHaptic('light');
              setVoiceNotice(`✓ "${target.note}" বাদ দেওয়া হয়েছে।`);
              speakAnnouncement(`${target.note} বাদ দেওয়া হয়েছে।`);
            } else {
              setVoiceNotice(`⚠️ "${res.removeItemName}" মেমোতে পাওয়া যায়নি।`);
            }
          }
        } else {
          if (cart.length === 0) {
            setVoiceNotice('⚠️ মেমোতে কোনো পণ্য নেই।');
            return;
          }
          if (res.removeItemName === '__last__' || !res.removeItemName) {
            ...
            playBeep(600);
            triggerHaptic('light');
            setVoiceNotice(`✓ সর্বশেষ পণ্যটি (${lastItem.product.banglaName || lastItem.product.name}) বাদ দেওয়া হয়েছে।`);
            speakAnnouncement('শেষের পণ্যটি বাদ দেওয়া হয়েছে।');
          } else {
            const q = res.removeItemName.toLowerCase();
            const target = cart.find(c =>
              (c.product.banglaName || '').toLowerCase().includes(q) ||
              (c.product.name || '').toLowerCase().includes(q)
            );
            if (target) {
              removeFromCart(target.product.id);
              playBeep(600);
              triggerHaptic('light');
              setVoiceNotice(`✓ "${target.product.banglaName || target.product.name}" মেমো থেকে বাদ দেওয়া হয়েছে।`);
              speakAnnouncement(`${target.product.banglaName || target.product.name} বাদ দেওয়া হয়েছে।`);
            } else {
              setVoiceNotice(`⚠️ "${res.removeItemName}" মেমোতে পাওয়া যায়নি।`);
            }
          }
      ...
      // 2. Clear Memo Command (e.g. "মেমো মুছো", "ক্লিয়ার করো")
      if (res.type === 'clear_memo') {
        if (posMode === 'calculator' || posMode === 'numpad') {
          handleNumpadClear();
        } else {
          clearCart();
        ...
        playBeep(500);
        triggerHaptic('medium');
        setVoiceNotice('✓ মেমোর সব পণ্য মুছে নতুন মেমো তৈরি হয়েছে।');
      ...
      // 3. Cash Checkout Command (e.g. "ক্যাশ বিক্রি", "বিল করো")
      if (res.type === 'cash_checkout') {
        if (posMode === 'calculator' || posMode === 'numpad') {
          handleNumpadCheckout('cash');
        } else {
      ...
      // 4. Due Checkout Command (e.g. "রহিম ভাই বাকি", "করিম এর বাকি")
      if (res.type === 'due_checkout') {
        if (res.customerName) {
          const matchCust = customers.find(c => (c.name || '').toLowerCase().includes(res.customerName!.toLowerCase()));
          if (matchCust) {
            setSelectedCustomer(matchCust);
            setNumpadCustomer(matchCust.id);
          } else {
            setNumpadNewCustName(res.customerName);
          }
        }
        if (posMode === 'calculator' || posMode === 'numpad') {
          handleNumpadCheckout('due');
        } else {
          handleQuickCheckout('due');
        }
        setExpressInput('');
        setExpressPreview(null);
        return;
      ...
      // 5. Discount Command (e.g. "২০ টাকা ছাড়")
      if (res.type === 'discount' && res.discountAmount) {
        setDiscount(res.discountAmount);
        playBeep(900);
        triggerHaptic('light');
        setVoiceNotice(`✓ ৳${res.discountAmount} ছাড় কার্যকর হয়েছে।`);
        speakAnnouncement(`${res.discountAmount} টাকা ছাড় দেওয়া হয়েছে।`);
        setExpressInput('');
        setExpressPreview(null);
        setTimeout(() => setVoiceNotice(''), 4000);
        return;
      ...
      if (res.type === 'add_items' && res.items && res.items.length > 0) {
        ...
        if (directProd) {
          if (posMode === 'calculator' || posMode === 'numpad') {
            ...
            addToCart(directProd, 1);
          }
          playBeep(1100);
          triggerHaptic('success');
          setVoiceNotice(`✓ ${directProd.banglaName || directProd.name} মেমোতে যোগ হয়েছে!`);
          setExpressInput('');
          setExpressPreview(null);
          setTimeout(() => setVoiceNotice(''), 4000);
        } else {
  ...
  // 🔮 Live preview of express item as the user types
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        const res = parseVoicePOSCommand(expressInput.trim(), products);
        if (res.type === 'add_items' && res.items && res.items.length > 0) {
          setExpressPreview(res.items[0]);
        } else {
          const q = expressInput.trim().toLowerCase();
          const p = products.find(prod => (prod.banglaName || '').toLowerCase().includes(q) || (prod.name || '').toLowerCase().includes(q));
          if (p) {
            setExpressPreview({
              name: p.name,
  ...
  }, [expressInput, products]);

  // 🎙️ Live Voice Speech Recognition for Express Bar
  const toggleExpressListening = () => {
    ...
    if (isExpressListening) {
      if (expressRecognitionRef.current) {
        try { expressRecognitionRef.current.stop(); } catch(e){}
      }
      setIsExpressListening(false);
      return;
    }
    ...
    try {
      ...
      expressRecognitionRef.current = rec;
      rec.start();
    } catch(e) {
      setIsExpressListening(false);
    }
  ...
  // ⚡ Top staple in-stock products for rapid 1-tap chips
  const topStapleProducts = useMemo(() => {
    return products
    ...
    <div className="app-container" style={{ paddingBottom: '160px' }}>
      ...
      {posMode === 'numpad' && (
        <div style={{
          ...
          {/* 🔮 REAL-TIME PRICE-TO-PRODUCT SMART SUGGESTIONS */}
          {numpadInput && predictedProducts.length > 0 && (
            <div style={{
                ...
                {predictedProducts.map((p) => {
                  ...
                  return (
                    <button
                      key={p.id}
                      type="button"
                      onClick={() => handleQuickProductTap(p)}
                      style={{
                        background: inStock ? '#ffffff' : '#fff1f2',
                        border: `1.5px solid ${inStock ? '#10b981' : '#f43f5e'}`,
                        borderRadius: '12px',
                        padding: '8px 12px',
                        cursor: 'pointer',
                        display: 'flex',
          ...
          {/* ⚡ If price entered but no direct stock match */}
          {numpadInput && predictedProducts.length === 0 && (
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                <button
                  type="button"
                  onClick={() => handleNumpadAddCurrent()}
                  style={{
                    background: '#4f46e5',
                    color: '#ffffff',
                    border: 'none',
                ...
                <button
                  type="button"
                  onClick={() => setShowNumpadProductDropdown(!showNumpadProductDropdown)}
                  style={{
                    background: 'var(--bg-canvas)',
          ...
          {/* 🌟 Popular In-Stock Products Pills (when idle/empty) */}
              ...
              <div style={{
                ...
                {popularNumpadProducts.map(p => (
                  ...
                  >
                    ...
                    <span style={{ color: '#059669', fontWeight: '900' }}>৳{p.sellingPrice}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {/* Items Pills (with quantity +/- & stock badge) */}
          {numpadItems.length > 0 && (
            <div style={{
              display: 'flex',
              flexDirection: 'column',
              ...
              {numpadItems.map((it) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', minWidth: 0 }}>
                    <span style={{
                      background: it.productId ? '#dcfce7' : '#f1f5f9',
                      color: it.productId ? '#15803d' : '#64748b',
                      fontSize: '9.5px',
                      padding: '1px 5px',
                      borderRadius: '4px',
                      fontWeight: '800',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '3px', background: 'var(--bg-card)', borderRadius: '6px', padding: '1px 4px', border: '1px solid var(--border-card)' }}>
                      <button
                        type="button"
                        onClick={() => updateNumpadItemQuantity(it.id, -1)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0 3px', fontSize: '13px', fontWeight: '900', color: '#475569' }}
                      >
                        -
                      </button>
                      <span className="num-font" style={{ fontSize: '11px', fontWeight: '800', minWidth: '14px', textAlign: 'center' }}>
                        {it.quantity || 1}
                      </span>
                      <button
                        type="button"
                    ...
                    <span className="num-font" style={{ fontWeight: '900', color: '#10b981', fontSize: '13px' }}>
                      ৳{it.amount}
                    </span>
